...
> Shiny apps are contained in a single script called `app.R`. The script `app.R` lives in a directory (for example, `newdir/`) and the app can be run with `runApp("newdir")`. `app.R` has three components: - a user interface (`ui`) object, that controls the layout and appearance of your app - a server function, that contains the instructions that your computer needs to build your app - a call to the shinyApp function, which creates Shiny app objects from an explicit UI/server pair
...
Shiny App: <https://bcgov-env.shinyapps.io/bc_wqg/> GitHub Repo: <https://github.com/bcgov/shinywqg>
